...
import { Edit, Fingerprint, X, Save } from 'lucide-react';
...
export default function EditScenarioModal({ scenario, onClose, onSave }: EditScenarioModalProps) {
  const [title, setTitle] = useState(scenario.title);
  const [code, setCode] = useState(scenario.code);
  const [desc, setDesc] = useState(scenario.desc);

  const handleSave = () => {
    onSave({ ...scenario, title, code, desc });
  };
      ...
      <div className="bg-surface-glass backdrop-blur-xl w-full max-w-lg border border-primary/40 shadow-[0_0_30px_rgba(13,242,13,0.15)] rounded-lg overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-300">
        <div className="px-6 py-4 border-b border-primary/20 flex items-center justify-between bg-surface-dark/40">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-white text-lg font-bold tracking-wide uppercase">Edit Scenario</h2>
          </div>
          <div className="text-[10px] text-primary/60 font-mono border border-primary/20 px-2 py-0.5 rounded">{scenario.id}</div>
        </div>
